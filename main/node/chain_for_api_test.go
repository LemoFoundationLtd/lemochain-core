	...
	defaultBlockInfos = []blockInfo{
		...
		// block 1 is stable block
		{
			...
			txList: []*types.Transaction{
				...
				signTransaction(types.NewTransaction(testAddr, defaultAccounts[0], common.Big1, 2000000, common.Big2, []byte{12}, 0, chainID, uint64(1538210391), "aa", string("aaa")), testPrivate),
...
func newDB() protocol.ChainDB {
	...
	for i, _ := range defaultBlockInfos {
		...
		if i == 0 || i == 1 {
			_, err := db.SetStableBlock(newBlock.Hash())
			if err != nil {
				panic(err)
			}
		}
...
func makeTransaction(fromPrivate *ecdsa.PrivateKey, to common.Address, amount *big.Int, gasPrice *big.Int, expiration uint64, gasLimit uint64) *types.Transaction {
	from := common.BytesToAddress(crypto.FromECDSA(fromPrivate))
	tx := types.NewTransaction(from, to, amount, gasLimit, gasPrice, []byte{}, 0, chainID, expiration, "", string("aaa"))
