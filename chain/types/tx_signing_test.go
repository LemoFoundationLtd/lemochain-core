...
func TestDefaultSigning(t *testing.T) {
	key, _ := crypto.GenerateKey()
	addr := crypto.PubkeyToAddress(key.PublicKey)

	signer := MakeSigner(TxVersion, chainId)
	tx, err := SignTx(NewTransaction(addr, new(big.Int), 0, new(big.Int), nil, 0, ExpirationFromNow(), "", nil), signer, key)
	if err != nil {
		t.Fatal(err)
	}
...
func TestDefaultChainId(t *testing.T) {
	...
	if tx.ChainId() != signer.chainId {
		t.Error("expected chainId to be", signer.chainId, "got", tx.ChainId())
	}
}

func TestDefaultSigningVitalik(t *testing.T) {
