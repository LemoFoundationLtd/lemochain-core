...
var (
	...
	reimbursementTx    = NewReimbursementTransaction(gasPayerAddr, common.HexToAddress("0x2"), common.Big2, []byte(bigString), 0, 200, 1544584596, "", "")
)

func TestNewReimbursementTransaction(t *testing.T) {
	rTx := reimbursementTx
	assert.Equal(t, uint64(0), rTx.Tx.GasLimit())
	assert.Equal(t, new(big.Int), rTx.Tx.GasPrice())
}
...
func TestTransaction_EncodeRLP_DecodeRLP(t *testing.T) {
	...
	assert.Equal(t, testTx.GasPayerSign(), result.GasPayerSign())
...
func TestReimbursementTransaction_EncodeRLP_DecodeRLP(t *testing.T) {
	...
	rb, err := rlp.EncodeToBytes(reimbursementTx.Tx)
	assert.NoError(t, err)
	result02 := Transaction{}
	err = rlp.DecodeBytes(rb, &result02)
	assert.NoError(t, err)
	assert.Equal(t, reimbursementTx.Tx.Type(), result02.Type())
	assert.Equal(t, reimbursementTx.Tx.Version(), result02.Version())
	assert.Equal(t, reimbursementTx.Tx.ChainID(), result02.ChainID())
	assert.Equal(t, reimbursementTx.Tx.To(), result02.To())
	assert.Equal(t, reimbursementTx.Tx.ToName(), result02.ToName())
	assert.Equal(t, reimbursementTx.Tx.GasPrice(), result02.GasPrice())
	assert.Equal(t, reimbursementTx.Tx.GasLimit(), result02.GasLimit())
	assert.Equal(t, reimbursementTx.Tx.Data(), result02.Data())
	assert.Equal(t, reimbursementTx.Tx.Expiration(), result02.Expiration())
	assert.Equal(t, reimbursementTx.Tx.Message(), result02.Message())
	assert.Equal(t, reimbursementTx.Tx.data.V, result02.data.V)
	assert.Equal(t, reimbursementTx.Tx.data.R, result02.data.R)
	assert.Equal(t, reimbursementTx.Tx.data.S, result02.data.S)
	assert.Equal(t, reimbursementTx.Tx.data.Hash, result02.data.Hash)
	assert.Equal(t, reimbursementTx.Tx.GasPayerSign(), result02.GasPayerSign())
	...
	assert.NoError(t, err)
	lastSignTx, err := MakeGasPayerSigner().GasPayerSignTx(firstSignTx, common.Big3, 3333, gasPayerPrivate)
	assert.NoError(t, err)
	...
	assert.Equal(t, "0xef8b94abf5a9922fad5965f3fb60293c53ec4c49c98bcd49ee07bf99fc61b40403cec5abc38dce624d510fb6a4211372eb453ab2acbcf0f7dc46589390e6b7c100", common.ToHex(lastSignTx.GasPayerSign()))
	rlpdata, err := rlp.EncodeToBytes(lastSignTx)
	assert.NoError(t, err)
	assert.Equal(t, "", common.ToHex(rlpdata))
	...
	assert.Equal(t, lastSignTx.GasPayerSign(), recovered.GasPayerSign())
...
func TestReimbursementTransaction(t *testing.T) {
	// without sign
	assert.Equal(t, "0x00761b3abdaabc466ebdf2fbb722d9be290b47a912286789d3def9f36e8e18cf", reimbursementTx.Tx.Hash().String())
	// two times sign
	h := MakeReimbursementTxSigner().Hash(reimbursementTx.Tx)
	// first sign
	sigData, err := crypto.Sign(h[:], testPrivate)
	assert.NoError(t, err)
	txV, err := reimbursementTx.Tx.WithSignature(MakeReimbursementTxSigner(), sigData)
	assert.NoError(t, err)
	// last sign
	txW, err := MakeGasPayerSigner().GasPayerSignTx(txV, common.Big3, 3333, gasPayerPrivate)
	assert.NoError(t, err)
	assert.Equal(t, "0xdc73856da0798e04e422347dd679a28b75a71c1ccc7d0b1d6d3899d3168452c6", txW.Hash().String())
}
...
func TestTransaction_MarshalJSON_UnmarshalJSON(t *testing.T) {
	...
	assert.NoError(t, err)
	assert.Equal(t, "", common.ToHex(data))
	...
	assert.NoError(t, err)
	assert.Equal(t, "", common.ToHex(data02))
	var txW *Transaction
...
func TestTransaction_MarshalJSON_UnmarshalJSON_bigTx(t *testing.T) {
	...
	data, err := json.Marshal(txV)
	assert.NoError(t, err)
	assert.Equal(t, "", common.ToHex(data))
	var parsedTx *Transaction
	err = json.Unmarshal(data, &parsedTx)
	assert.NoError(t, err)
	assert.Equal(t, txV.Hash(), parsedTx.Hash())
