...
func TestNewContractCreation(t *testing.T) {
	expiration := ExpirationFromNow()
	tx := NewContractCreation(testAddr, common.Big1, 100, common.Big2, []byte{12}, 0, 200, expiration, "aa", "")
	assert.Equal(t, params.OrdinaryTx, tx.Type())
	assert.Equal(t, TxVersion, tx.Version())
	assert.Equal(t, uint16(200), tx.ChainID())
...
func TestNoReceiverTransaction(t *testing.T) {
	from, _ := common.StringToAddress("Lemo83GN72GYH2NZ8BA729Z9TCT7KQ5FC3CR6DJG")
	privFrom, _ := crypto.ToECDSA(common.FromHex("0xc21b6b2fbf230f665b936194d14da67187732bf9d28768aef1a3cbb26608f8aa"))
	to, _ := common.StringToAddress("Lemo842BJZ4DKCC764C63Y6A943775JH6NQ3Z33Y")
	amount := big.NewInt(999)
	data := common.FromHex("")
	tx := NewTransaction(from, to, amount, uint64(50000), common.Big1, data, 0, 100, uint64(time.Now().Unix()+30*60), "", "")
	sigTx, _ := MakeSigner().SignTx(tx, privFrom)
	by, _ := sigTx.MarshalJSON()
	t.Log(by)
	ttx := &Transaction{}
	json.Unmarshal(by, ttx)
	t.Log(ttx.String())
}
