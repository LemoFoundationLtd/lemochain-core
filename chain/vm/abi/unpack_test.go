...
var unpackTests = []unpackTest{
	...
	// multi dimensional, if these pass, all types that don't assert length prefix should pass
	{
		def:  `[{"type": "uint8[][]"}]`,
...
func methodMultiReturn(assert *assert.Assertions) (ABI, []byte, methodMultiOutput) {
	const definition = `[
...
func TestMethodMultiReturn(t *testing.T) {
	...
	abi, data, expected := methodMultiReturn(assert.New(t))
	bigint := new(big.Int)
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.error == "" {
				assert.Nil(err, "Should be able to unpack method outputs.")
