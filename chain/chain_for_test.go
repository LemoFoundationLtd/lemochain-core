...
func newChain() *BlockChain {
	...
	bc, err := NewBlockChain(big.NewInt(1), db, recvBlockCh)
	if err != nil {
		panic(err)
	}
	return bc
...
func saveBlock(db protocol.ChainDB) {
	...
	tr, err := trie.NewSecure(common.Hash{}, trieDB, account.MaxTrieCacheGen)
	if err != nil {
		panic(err)
	} else {
...
func testStorageTrieGet(db protocol.ChainDB) {
	tr, err := trie.NewSecure(defaultAccounts[0].StorageRoot, db.GetTrieDatabase(), account.MaxTrieCacheGen)
	if err != nil {
		panic(err)
	} else {
