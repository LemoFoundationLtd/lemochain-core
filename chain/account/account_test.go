...
func TestAccount_Finalise_Save(t *testing.T) {
	...
	assert.Equal(t, 0, len(account.dirtyStorage))
	account.SetVersion(BalanceLog, 11)
	err = account.Finalise(blockHeight)
	assert.Equal(t, 3, len(account.data.NewestRecords))
	assert.Equal(t, blockHeight, account.data.NewestRecords[BalanceLog].Height)
	assert.Equal(t, uint32(11), account.data.NewestRecords[BalanceLog].Version)
	account.SetVersion(StorageLog, 12)
	err = account.Finalise(blockHeight + 1)
	assert.Equal(t, 3, len(account.data.NewestRecords))
	assert.Equal(t, blockHeight+1, account.data.NewestRecords[StorageLog].Height)
	assert.Equal(t, uint32(12), account.data.NewestRecords[StorageLog].Version)
	// save
	err = account.Save()
	assert.NoError(t, err)
