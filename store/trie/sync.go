...
type TrieSync struct {
	database store.DatabaseReader     // Persistent database to check for existing entries
...
func NewTrieSync(root common.Hash, database store.DatabaseReader, callback LeafCallback) *TrieSync {
	ts := &TrieSync{
		database: database,
...
// Process injects a batch of retrieved trie nodes data, returning if somlemoing
...
func (s *TrieSync) Commit(dbw store.Putter) (int, error) {
	...
	for i, key := range s.membatch.order {
		if err := dbw.Put(0, key[:], s.membatch.batch[key]); err != nil {
			return i, err
		}
	}
