...
func makeTestTrie() (*store.TrieDatabase, *Trie, map[string][]byte) {
...
// content map.
func checkTrieContents(t *testing.T, db *store.TrieDatabase, root []byte, content map[string][]byte) {
...
func checkTrieConsistency(db *store.TrieDatabase, root common.Hash) error {
...
func TestEmptyTrieSync(t *testing.T) {
	diskdbA, _ := store.NewMemDatabase()
	triedbA := store.NewTrieDatabase(diskdbA)

	diskdbB, _ := store.NewMemDatabase()
	triedbB := store.NewTrieDatabase(diskdbB)
...
func testIterativeTrieSync(t *testing.T, batch int) {
	...
	for len(queue) > 0 {
		...
		// if index, err := sched.Commit(); err != nil {
