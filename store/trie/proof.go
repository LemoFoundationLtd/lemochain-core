...
func VerifyProof(rootHash common.Hash, key []byte, proofDb store.DatabaseReader) (value []byte, err error, nodes int) {
	key = keybytesToHex(key)
	wantHash := rootHash
	for i := 0; ; i++ {
		buf, _ := proofDb.Get(leveldb.ItemFlagTrie, wantHash[:])
		if buf == nil {
